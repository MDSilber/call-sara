...
PLAID_OVERLAP = {
    ...
    "investments": [{
        ...
        "total_investment_transactions": 8,
    }],
...
class TestVanguardOverlapRegression:
    ...
    def test_every_overlap_dedupes_with_the_tier_named(self, fresh_vault: Path,
        ...
        assert "fetched 8 -> mapped 8 | unmapped 0  ✓ counts reconcile" in r.stdout
        assert "new 2 (cash effect -89,171.21 USD), deduped 6" in r.stdout
    ...
    def test_the_genuinely_new_row_still_imports_with_the_gap_named(
        ...
        seed(fresh_vault)
        r = run_overlap_ingest(fresh_vault, tmp_path)
        assert "new 2 (cash effect -89,171.21 USD)" in r.stdout
    ...
    def test_without_the_seeded_ledger_everything_imports(self, fresh_vault: Path,
                                                          tmp_path: Path) -> None:
        # no OFX history -> nothing to corroborate against -> all eight are new
        r = run_overlap_ingest(fresh_vault, tmp_path)
        assert "new 8" in r.stdout and INVEST_TIER_LABEL not in r.stdout
...
''')
    ...
    def test_income_never_claims_a_plain_buy(self, fresh_vault: Path) -> None:
        seed(fresh_vault, '''
...
''')
        ...
        assert d.check_invest(date(2026, 7, 15), D("103.17"), "DIV VMFXX", "pl-3",
                              kind="INCOME", ticker="VMFXX", units=D("0"),
